...
local theme = {
	...
	insert = { a = { fg = "#ffffff", bg = colors.light_green } },
	...
	sections = process_sections({
		...
		lualine_b = {
			...
			-- "diff",
			{
				...
				source = { "nvim" },
				sections = { "error" },
				diagnostics_color = { error = { bg = "#ff0000", fg = colors.white } },
			},
			{
				"diagnostics",
				source = { "nvim" },
				sections = { "warn" },
				diagnostics_color = { warn = { bg = "#ffff00", fg = colors.white } },
			},
		...
		-- lualine_z = { '%l:%c', '%p%%/%L' },
		lualine_z = { { "%l:%c", color = { bg = colors.grey, fg = "#ffffff" } }, "%p%%" },
	}),
	inactive_sections = {
		lualine_c = { "%f %y %m" },
		lualine_x = {},
